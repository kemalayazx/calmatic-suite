...
export default function FuelPage() {
  ...
  const tabs = ["Fuel Cost", "Trip Splitter", "MPG ↔ L/100km"];
  ...
  const distLabel = distanceUnit === "miles" ? "miles" : "km";
  ...
  return (
    <div style={{ maxWidth: "800px", margin: "0 auto" }}>
      <h1 style={{ fontSize: "2rem", fontWeight: 800, marginBottom: "0.5rem" }}>Fuel Calculator</h1>
      <p style={{ color: "#71717a", marginBottom: "2rem" }}>Trip fuel cost, cost splitter, and MPG/L converter.</p>
      ...
      {(activeTab === 0 || activeTab === 1) && (
        <>
          ...
          <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.5rem", flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: "0.8rem", color: "#71717a" }}>Distance:</span>
            <ToggleBtn value="miles" current={distanceUnit} set={(v) => setDistanceUnit(v as DistanceUnit)} label="Miles" />
            <ToggleBtn value="km" current={distanceUnit} set={(v) => setDistanceUnit(v as DistanceUnit)} label="km" />
            <span style={{ fontSize: "0.8rem", color: "#71717a", marginLeft: "0.5rem" }}>Efficiency:</span>
            ...
            <span style={{ fontSize: "0.8rem", color: "#71717a", marginLeft: "0.5rem" }}>Price:</span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: activeTab === 1 ? "1fr 1fr 1fr 1fr" : "1fr 1fr 1fr", gap: "1rem", marginBottom: "2rem" }}>
            <div>
              <label style={{ display: "block", color: "#a1a1aa", fontSize: "0.8rem", marginBottom: "0.3rem" }}>Distance ({distLabel})</label>
            ...
            <div>
              <label style={{ display: "block", color: "#a1a1aa", fontSize: "0.8rem", marginBottom: "0.3rem" }}>Fuel Efficiency ({effLabel})</label>
              <input type="number" value={fuelEfficiency} onChange={(e) => setFuelEfficiency(e.target.value)}
                style={{ width: "100%", padding: "0.5rem 0.75rem", borderRadius: "0.5rem", border: "1px solid #3f3f46", background: "#18181b", color: "#fafafa", fontSize: "0.9rem" }} />
            </div>
            <div>
              <label style={{ display: "block", color: "#a1a1aa", fontSize: "0.8rem", marginBottom: "0.3rem" }}>Gas Price ({priceLabel})</label>
              <input type="number" value={gasPrice} onChange={(e) => setGasPrice(e.target.value)} step="0.01"
                style={{ width: "100%", padding: "0.5rem 0.75rem", borderRadius: "0.5rem", border: "1px solid #3f3f46", background: "#18181b", color: "#fafafa", fontSize: "0.9rem" }} />
            </div>
            {activeTab === 1 && (
              <div>
                <label style={{ display: "block", color: "#a1a1aa", fontSize: "0.8rem", marginBottom: "0.3rem" }}>Passengers</label>
      ...
      {activeTab === 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          {[
            { label: "Fuel Needed", value: `${costResult.fuelNeeded.toFixed(2)} ${costResult.fuelUnit}`, color: "#a78bfa" },
            { label: "Total Cost", value: `$${costResult.totalCost.toFixed(2)}`, color: "#22c55e" },
      ...
      {activeTab === 1 && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem" }}>
          {[
            { label: "Fuel Needed", value: `${splitResult.fuelNeeded.toFixed(2)} ${splitResult.fuelUnit}`, color: "#a78bfa" },
            { label: "Total Fuel Cost", value: `$${splitResult.totalCost.toFixed(2)}`, color: "#fbbf24" },
            { label: `Cost per Person (÷${passengers})`, value: `$${splitResult.costPerPerson.toFixed(2)}`, color: "#22c55e" },
          ].map(({ label, value, color }) => (
            <div key={label} style={{ background: "#18181b", border: "1px solid #27272a", borderRadius: "0.75rem", padding: "1.5rem", textAlign: "center" }}>
              <div style={{ fontSize: "0.8rem", color: "#71717a", marginBottom: "0.5rem" }}>{label}</div>
      ...
      {activeTab === 2 && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem" }}>
          <div style={{ background: "#18181b", border: "1px solid #27272a", borderRadius: "0.75rem", padding: "1.5rem" }}>
            <div style={{ fontWeight: 700, color: "#a78bfa", marginBottom: "1.25rem" }}>MPG → L/100km</div>
            <div style={{ marginBottom: "1rem" }}>
              <label style={{ display: "block", color: "#a1a1aa", fontSize: "0.8rem", marginBottom: "0.3rem" }}>Miles per Gallon</label>
          ...
          <div style={{ background: "#18181b", border: "1px solid #27272a", borderRadius: "0.75rem", padding: "1.5rem" }}>
            <div style={{ fontWeight: 700, color: "#a78bfa", marginBottom: "1.25rem" }}>L/100km → MPG</div>
            <div style={{ marginBottom: "1rem" }}>
              <label style={{ display: "block", color: "#a1a1aa", fontSize: "0.8rem", marginBottom: "0.3rem" }}>Liters per 100km</label>
