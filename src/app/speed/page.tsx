...
export default function SpeedPage() {
  ...
  const modeConfig: Record<Mode, { label: string; in1Label: string; in1Units: string[]; in2Label: string; in2Units: string[] }> = {
    speed: { label: "Distance + Time → Speed", in1Label: "Distance", in1Units: DIST_UNITS, in2Label: "Time", in2Units: TIME_UNITS },
    distance: { label: "Speed + Time → Distance", in1Label: "Speed", in1Units: SPEED_UNITS, in2Label: "Time", in2Units: TIME_UNITS },
    time: { label: "Speed + Distance → Time", in1Label: "Speed", in1Units: SPEED_UNITS, in2Label: "Distance", in2Units: DIST_UNITS },
  };
  ...
  return (
    <div style={{ maxWidth: "700px", margin: "0 auto" }}>
      ...
      <p style={{ color: "#71717a", marginBottom: "2rem" }}>Enter any two values to calculate the third.</p>
      ...
      <div style={{ background: "#18181b", border: "1px solid #3f3f46", borderRadius: "0.75rem", padding: "1.5rem", marginBottom: "1.5rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "0.75rem", marginBottom: "1rem" }}>
          <NumIn label={`${cfg.in1Label} value`} value={v1} onChange={setV1} />
          <Select label="Unit" value={u1} onChange={setU1} options={cfg.in1Units} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "0.75rem", marginBottom: "1.5rem" }}>
          <NumIn label={`${cfg.in2Label} value`} value={v2} onChange={setV2} />
          <Select label="Unit" value={u2} onChange={setU2} options={cfg.in2Units} />
        </div>

        {/* Output units */}
        <div style={{ borderTop: "1px solid #27272a", paddingTop: "1rem" }}>
          <div style={{ fontSize: "0.75rem", color: "#71717a", marginBottom: "0.5rem", textTransform: "uppercase", letterSpacing: "0.06em" }}>Output Units</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.5rem" }}>
            <Select label="Speed" value={outSpeed} onChange={(v) => setOutSpeed(v as SpeedUnit)} options={SPEED_UNITS} />
            <Select label="Distance" value={outDist} onChange={(v) => setOutDist(v as DistanceUnit)} options={DIST_UNITS} />
            <Select label="Time" value={outTime} onChange={(v) => setOutTime(v as TimeUnit)} options={TIME_UNITS} />
          </div>
        </div>
      </div>
      ...
      {result && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginBottom: "2rem" }}>
          <ResultCard label="Speed" value={result.speed.toFixed(2)} unit={outSpeed} color={mode === "speed" ? "#a78bfa" : "#71717a"} />
          <ResultCard label="Distance" value={result.distance.toFixed(2)} unit={outDist} color={mode === "distance" ? "#22c55e" : "#71717a"} />
          <ResultCard label="Time" value={result.timeDisplay.split(" ")[0]} unit={result.timeDisplay.split(" ").slice(1).join(" ") || outTime} color={mode === "time" ? "#f97316" : "#71717a"} />
        </div>
      )}

      {/* Pace Calculator */}
      <div style={{ background: "#18181b", border: "1px solid #3f3f46", borderRadius: "0.75rem", padding: "1.5rem" }}>
        <h2 style={{ fontWeight: 700, color: "#fafafa", marginBottom: "1rem", fontSize: "1.1rem" }}>Running Pace Calculator</h2>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "0.75rem", marginBottom: "1rem" }}>
          <NumIn label="Pace (minutes)" value={paceVal} onChange={setPaceVal} />
          <Select label="Pace Unit" value={paceInput} onChange={(v) => setPaceInput(v as "min/km" | "min/mi")} options={["min/km", "min/mi"]} />
        </div>
        <Select label="Race Distance" value={RACES[raceIdx].label} onChange={(v) => setRaceIdx(RACES.findIndex((r) => r.label === v))} options={RACES.map((r) => r.label)} />
      ...
      <p style={{ color: "#52525b", fontSize: "0.8rem", textAlign: "center", marginTop: "1.5rem" }}>
        Calculations use standard unit conversion factors. Results are for informational purposes only.
      </p>
    </div>
  );
}
